...
our $DRIVER_ROOT = (defined($ENV{'DRIVER_ROOT'}) ? $ENV{'DRIVER_ROOT'} :
  die "FATAL ERROR: $0 - You must set DRIVER_ROOT to the root directory of the harness");

unshift( @INC, "$ROOT/" );
unshift( @INC, ".");
unshift( @INC, "$DRIVER_ROOT/");
